...
typealias RegistrarEntry<T> = Miehex_revolutionRegistrar<T>.Entry<out T>

abstract class Miehex_revolutionRegistrar<T : Any>(
    ...
    fun <V : T> register(name: String, builder: () -> V): Entry<V> = register(Miehex_revolution.id(name), builder)
    ...
    open inner class Entry<V : T>(
        ...
        override fun equals(other: Any?) = when (other) {
            is Miehex_revolutionRegistrar<*>.Entry<*> -> key.registry().equals(other.key.registry()) && id.equals(other.id)
            else -> false
        }
